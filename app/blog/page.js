...
import { getBlogCardExcerpt } from '@/lib/blogContent';
...
export default async function Blog() {
  ...
  try {
    ...
    if (page?.sections) {
      page.sections.forEach((sec) => {
        if (sec.key === 'header' && sec.content) header = { ...header, ...sec.content };
        if (sec.key === 'cta' && sec.content) cta = { ...cta, ...sec.content };
      });
    }
  } catch (err) {
    console.warn('[Blog] database fetch failed, using fallback content.');
  }
  ...
  const formattedPosts = posts.map((post) => ({
    ...
    excerpt: getBlogCardExcerpt(post.excerpt, post.body),
    ...
    <>
      <section
        style={{
          ...
          padding: '9rem 0 6rem',
          background:
            'linear-gradient(rgba(4, 13, 26, 0.85), rgba(11, 31, 51, 0.95)), url(/ev_charging_business.png)',
      ...
      >
        <div className="container">
          <div>
            <h2 style={{ color: '#fff', marginBottom: '0.75rem' }}>{cta.title}</h2>
