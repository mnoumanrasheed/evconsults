...
import { prepareBlogPostForSave } from '@/lib/blogContent';
...
export async function createBlogPost(data) {
  ...
  const prepared = prepareBlogPostForSave(data);
  const parsed = blogPostSchema.safeParse(prepared);
  if (!parsed.success) return { error: parsed.error.errors[0].message };

  if (!parsed.data.excerpt || parsed.data.excerpt.length < 10) {
    return { error: 'Could not generate a valid excerpt. Add more body content or write a summary.' };
  }

  try {
    const slug = prepared.slug || slugify(prepared.title);
