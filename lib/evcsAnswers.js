...
**Practical IESCO / NEPRA Compliance Point:**
Treat this as a **commercial electricity connection** plus NEPRA safety, tariff, metering and operational requirements.${DISCLAIMER}`;
  }
...
Include earthing design in your IESCO submission — heavily reviewed alongside RCCB/RCD/ELCB protection.${DISCLAIMER}`;
  ...
  if (
    ...
    return `**Direct Answer:**
EVSE must include comprehensive electrical protection per NEPRA regulations.
...
Load feasibility is the **#1 approval priority**.${DISCLAIMER}`;
  ...
  if (
    ...
    return `**Direct Answer:**
Installation must follow NEPRA practical requirements.
...
Poor records create compliance issues during inspections.${DISCLAIMER}`;
  ...
  if (matchesAny(['ocpp', 'smart charging', 'ems', 'energy management', 'network connectivity', 'fault detection'])) {
    return `**Direct Answer:**
EVSE should support smart operations under NEPRA regulations.
...
- **Tariff/margin violation:** NEPRA${DISCLAIMER}`;
  ...
  if (
    ...
    return `**Key Definitions (NEPRA Regulations):**

- **EV:** Electric vehicle using electric motor and rechargeable battery.
- **EVSE:** Electric Vehicle Supply Equipment (charger, connectors, protection, wiring, and related system).
...
PEVCS falls under the **Commercial Consumer Category** under NEPRA regulations.${DISCLAIMER}`;
  }
...
Include manufacturer compliance documents and IP/IK certificates in your submission file.${DISCLAIMER}`;
  }
...
6. Complete documentation (SLD, load calc, site plan, datasheet, safety plan)${DISCLAIMER}`;
  ...
  if (
    ...
    return `**Direct Answer:**
**NEPRA regulations** regulate Public EV Charging Stations (PEVCS). IESCO connection is treated as a **Commercial Consumer** application plus NEPRA safety, metering, billing, and operational rules.
...
**Important Note:**
EVConsults supports IESCO files, NEPRA compliance, and commissioning. Please [Contact Us](/contact) for an expert review.${DISCLAIMER}`;
  }
